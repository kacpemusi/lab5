...
{
if(mm_in.m1_i1 == FIRST)
	am = mm_in.m1_i1;
if(mm_in.m1_i1 == WORST)
	am = mm_in.m1_i1;
...
{
register struct hole *ho;
phys_clicks buff[1+NR_HOLES*2];
int i = 0;
int p = 0;
ho = hole_head;
while(ho->h_base < swap_base && ho != NIL_HOLE && i<(mm_in.m1_i1/sizeof(phys_clicks)))
{
buff[i++] = ho->h_len;
buff[i++] = ho->h_base;
p++;
...
buff[i]=0;
sys_copy(MM_PROC_NR, D, (phys_bytes) buff, mm_in.m_source, D, (phys_bytes)mm_in.m1_p1, mm_in.m1_i1);
return p;
}
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *pm, *m;
...
if(m->h_len >= clicks && hp == NIL_HOLE){
	old_base = m->h_base;
	m->h_base += clicks;
	m->h_len -= clicks;
	if(hp->h_len == 0) del_slot(pm,m);
	return(old_base);}
}while(swap_out());}
